...
/// @version $Id: MSLane.h 8617 2010-04-21 15:06:43Z behrisch $
///
...
public:
    ...
    friend class GUILaneWrapper;//?? ACE ??

    friend class MSXMLRawOut;
    
    friend class MSStrip;
...
public:
     ...
     */
    MSLane(const std::string &id, SUMOReal maxSpeed, SUMOReal length, size_t stripWidth,
           MSEdge * const edge,
    ...
    virtual bool isEmissionSuccess(MSVehicle* vehicle, SUMOReal speed, SUMOReal pos,
                                   bool recheckNextLanes, size_t startStripId=0) throw(ProcessError);
    ...
    //?? ACE ?? only for gui
    virtual const std::deque< MSVehicle* > &getVehiclesSecure(int) const throw();
    ...
    /** @brief Returns the width of the lane in meters
     * @return The width of the lane
     */
    SUMOReal getRealLaneWidth() const;

    /** @brief Returns the number of strips in the lane
    ...
    void printDebugMsg(const std::string &msg="") const;
protected:
    ...
    virtual bool push(MSVehicle* veh, const std::vector<int> &stripID, bool hasMainStrip);
...
protected:
    ...
    /// Lane width [in number of strips]
...
private:
    ...
    // Function object for comparing if two vehicles are the same
    struct VehInfoEqComparator : public std::binary_function< std::pair<MSVehicle *, SUMOReal>, 
                std::pair<MSVehicle *, SUMOReal>, bool > {
        bool operator()(std::pair<MSVehicle *, SUMOReal> veh1, std::pair<MSVehicle *, SUMOReal> veh2) const {
            return (veh1.first == veh2.first);
        }
    };
    struct VehInfoLessComparator : public std::binary_function< std::pair<MSVehicle *, SUMOReal>, 
                std::pair<MSVehicle *, SUMOReal>, bool > {
        bool operator()(std::pair<MSVehicle *, SUMOReal> veh1, std::pair<MSVehicle *, SUMOReal> veh2) const {
            return (veh1.second <= veh2.second);
        }
    };
