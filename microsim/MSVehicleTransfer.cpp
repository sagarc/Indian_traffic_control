...
/// @date    Sep 2003
/// @version $Id: MSVehicleTransfer.cpp 8771 2010-05-24 14:50:47Z dkrajzew $
///
...
MSVehicleTransfer::checkEmissions(SUMOTime time) throw() {
    ...
    for (VehicleInfVector::iterator i=myVehicles.begin(); i!=myVehicles.end();) {
      ...
      /*
        ashutosh modification
        MSLane::StripCont strips = l->getMyStrips();

        for (MSLane::StripContConstIter it=strips.begin(); it != strips.end(); ++it)
        {     std::cerr<<(*it)->getNumericalID()<<"ashutosh" ;
        	if ((*it)->freeEmit(*(desc.myVeh), MIN2(l->getMaxSpeed(), desc.myVeh->getMaxSpeed()))) {
