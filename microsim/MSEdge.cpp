...
/// @version $Id: MSEdge.cpp 8617 2010-04-21 15:06:43Z behrisch $
///
...
MSEdge::initialize(MSLane* departLane,
    ...
    for (std::vector<MSLane*>::iterator lane = lanes->begin(); lane != lanes->end(); ++lane) {
        for (int id = 0; id < (*lane)->getWidth(); ++id) {
            MSStrip* strip = (*lane)->getStrip(id);
            strips.push_back(strip);
        }
    }
    myFunction = function;
    if (myLanes && myLanes->size() > 1 && function!=EDGEFUNCTION_INTERNAL) {
        myLaneChanger = new MSStripChanger(&strips);
...
MSEdge::getDepartLane(const MSVehicle &v) const throw() {
    ...
    case DEPART_LANE_GIVEN:
        //TODO: add check for vehicle width vs no. of strips here
