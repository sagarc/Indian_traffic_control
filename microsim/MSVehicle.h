...
/// @version $Id: MSVehicle.h 8758 2010-05-19 09:47:17Z dkrajzew $
///
...
public:
    ...
    /// Finds the vehicle in its strips closest to it (and in front)
    ...
    const StripCont &getStrips() const {
        return myStrips;
    }
    
    std::vector<int> getStripIDs() const;
    
    void enterStripAtMove(MSStrip *strip);
    
    void printDebugMsg(const std::string &s="") const;
    
    std::deque< MSVehicle* >::iterator eraseFromStrips(MSStrip*);
     ...
     */
    // Position on Lane is position on Lane which contains mainStrip
     ...
     */
    void enterLaneAtMove(MSLane* enteredLane, SUMOReal driven, const StripCont &strips, bool hasMain=false);
     ...
     */
    void enterLaneAtEmit(MSLane* enteredLane, SUMOReal pos, SUMOReal speed, StripCont &strips);
    ...
    /** @brief Update when the vehicle enters a new strip
     *  @param[in] strips The strips that the vehicle moves into*/
    void enterStripsAtStripChange(const StripCont &strips);
    ...
    /// Returns the lanes of all the vehicles strips along with the strip IDs of 
    /// the strips each vehicle occupies for the corresponding lane
    const std::vector<std::vector<int> > getLanes() const;

    /// Returns the main (owner) strip of the vehicle (the first (leftmost) strip)
    const MSStrip &getMainStrip() const;
    
    /// Returns the main (owner) strip's id (the first (leftmost) strip)?? ACE ??
    virtual size_t getMainStripNumericalID() const;


    /// Returns the left most strip of the vehicle
    const MSStrip *getLeftStrip() const;
    
    /// Returns the right most strip of the vehicle (may be on a different
    /// lane than the left most strip)
    const MSStrip *getRightStrip() const;
    
    bool isMainStrip(const MSStrip &strip) const;
    
    virtual SUMOReal getRealWidth() const;//?? ACE ?? made virtual
    
    size_t getWidth() const {
        assert(myWidth != 0 && myWidth < 10); // XXX: temporarily 10, proper check later
        return myWidth;
    }
...
protected:
    ...
    /// The lane the vehicle is on (Lane of the mainStrip)
    MSLane* myLane;

    // The strips that the vehicle occupies
    StripCont myStrips;
    
    // The (integral) number of strips that a vehicle occupies
    size_t myWidth;
