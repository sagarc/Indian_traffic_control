...
/// @date    Sept 2002
/// @version $Id: GUIVehicle.cpp 8725 2010-05-04 20:27:48Z behrisch $
///
...
#include <microsim/MSStrip.h>//?? ACE ??
#include "GUINet.h"
...
#include <utils/iodevices/OutputDevice_File.h> //?? ACE ??
...
inline void
drawAction_drawVehicleAsQuadPlus(const GUIVehicle &veh, SUMOReal upscale) {
    SUMOReal length = veh.getVehicleType().getLength();
    SUMOReal width = veh.getRealWidth();//?? ACE ??
    SUMOReal halfwidth = width/2;//?? ACE ?? adjust vehicle width according to type
    glPushMatrix();
    glScaled(upscale, upscale, 1);
    if (length<8) {
        glScaled(1, length - 0.1*length, 1);//?? ACE - 0.1*length??
        /*glBegin(GL_TRIANGLES);
        glVertex2d(0, 0);*/ //?? ACE ??
        glBegin(GL_QUADS);//?? ACE ??
        glVertex2d(0-halfwidth, 1);//?? ACE ??
        glVertex2d(0+halfwidth-0.1*halfwidth, 1);//?? ACE ??
        glVertex2d(0+halfwidth-0.1*halfwidth, 0);//?? ACE ??
        glVertex2d(0-halfwidth, 0);//?? ACE ??
        glEnd();
    } else {
        glBegin(GL_TRIANGLES);
        glVertex2d(0, 0);
        glVertex2d(0-halfwidth, 0+2);//?? ACE ??
        glVertex2d(0+halfwidth, 0+2);//?? ACE ??
        glVertex2d(0-halfwidth, 2);//?? ACE ??
        glVertex2d(0-halfwidth, length);//?? ACE ??
        glVertex2d(0+halfwidth, length);//?? ACE ??
        glVertex2d(0+halfwidth, 2);//?? ACE ??
        glVertex2d(0-halfwidth, 2);//?? ACE ??
        glVertex2d(0+halfwidth, length);//?? ACE ??
...
drawAction_drawVehicleAsBoxPlus(const GUIVehicle &veh, SUMOReal upscale) {
    ...
    glScaled(1, veh.getRealWidth(), 1.);//?? ACE ??
    glScaled(upscale, upscale, 1);
...
drawAction_drawVehicleAsPoly(const GUIVehicle &veh, SUMOReal upscale) {
    ...
    glRotated(90, 0, 0, 1);
    glTranslated(veh.getVehicleType().getGuiOffset(), 0, 0);
    glScaled(length-veh.getVehicleType().getGuiOffset(), veh.getRealWidth()/*veh.getVehicleType().getGuiWidth()*/, 1.);//?? ACE ?? getRealWidth instead of getGuiWidth
    glScaled(upscale, upscale, 1);
    ...
    case SVS_MOTORCYCLE: {
        ...
        glTranslated(0, 0, -.045);
        GLHelper::drawFilledCircle(0.1);//?? ACE ??
        glScaled(.7, 2, 1);
        glTranslated(0, 0, .045);
        glTranslated(0, 0, -.04);
        glColor3dv(lighter);
    ...
    case SVS_PASSENGER_WAGON:
        drawPoly(vehiclePoly_PassengerCarBody, -4);//?? ACE ??
        glColor3dv(lighter);
        drawPoly(vehiclePoly_PassengerCarBodyFront, -4.5);
        glColor3d(0, 0, 0);
        drawPoly(vehiclePoly_PassengerFrontGlass, -4.5);
...
drawAction_drawVehicleBlinker(const GUIVehicle &veh) {
    double dir = (double) veh.getCORNIntValue(MSCORN::CORN_VEH_BLINKER)*veh.getRealWidth()/*veh.getVehicleType().getGuiWidth()*/*.5;//?? ACE ??
    if (dir==0) {
        return;
    }
    if (veh.getRealWidth()<.5/*veh.getVehicleType().getGuiWidth()<.5*/) { //?? ACE ??
        return;
    }
    glColor3d(1.f, .8f, 0);
    glPushMatrix();
    glTranslated(dir, BLINKER_POS_FRONT+veh.getVehicleType().getGuiOffset(), 0);
    GLHelper::drawFilledCircle(.3, 6);//?? ACE ?? .5 changed to ___(x-coordinate)
    glPopMatrix();
    glPushMatrix();
...
GUIVehicle::drawGL(const GUIVisualizationSettings &s) const throw() {
    ...
    Position2DVector temp = myLane->getShape();//?? ACE ??
    /*?? ACE ??*/
	if (!(temp.myCont.size() == 2 && temp.myCont[0] == temp.myCont[1])) {//?? to correct a specific condition (for internal lane the condition holds true)
		if(getWidth() > 1)
		{
			temp.move2side((SUMOReal)1.65 - (SUMOReal)SUMO_const_laneWidth/myLane->getWidth()*(1+ getMainStripNumericalID())
										  + (SUMOReal)SUMO_const_laneWidth/(2*myLane->getWidth())
										  - (SUMOReal)(getWidth()-1)/2*(SUMO_const_laneWidth/myLane->getWidth()));
		}
		else
		{
			temp.move2side((SUMOReal)+1.65 - (SUMOReal)SUMO_const_laneWidth/myLane->getWidth()*(1+ getMainStripNumericalID())
    ...
    Position2D pTemp = temp.positionAtLengthPosition(myState.pos());/*?? ACE ??*/
    //std::cout << p1.y()<<"PPPPPPPPPPPPPP";
    //p1.y() = p1.y()/myLane->getWidth()*(1+ myLane->getNumericalID());//?? ACE ??
    Position2D p2 = myFurtherLanes.size()>0
                    ...
                    : myLane->getShape().positionAtLengthPosition(myState.pos()-myType->getLength());//?? ACE ?? myLane replaced with getMainStrip
    /*if(getWidth() > 1)
    	glTranslated(p1.x(),(p1.y()-1.65 + SUMO_const_laneWidth/myLane->getWidth()*(1+ getMainStripNumericalID())-SUMO_const_laneWidth/(2*myLane->getWidth()))+(getWidth()-1)/2*(SUMO_const_laneWidth/myLane->getWidth()), 0);//?? ACE ??
    else
    	glTranslated(p1.x(),p1.y()-1.65 + SUMO_const_laneWidth/myLane->getWidth()*(1+ getMainStripNumericalID())-SUMO_const_laneWidth/(2*myLane->getWidth()), 0);//?? ACE ??*/

    /*
     * Print gui dump
     * ofstream out;
    out.open("gui_dump.xml", ios::app);
       if( !out )
       {
          std::cout << "Couldn't open file."  << std::endl;
       }
       SUMOTime t = MSNet::getInstance()->getCurrentTimeStep();
    out <<std::endl <<"<vehicle id="<<"\""<< this->getID()<<"\""<< " lane_info=" <<"\""<< this->getLane().getID()<<":"<<this->getMainStripNumericalID()<<"\"" ;
    out << " xpos="<<"\""<< pTemp.x() <<"\""<< " ypos=" <<"\""<< pTemp.y() <<"\""<<" time="<<"\""<<t/1000<<"\""<< "/>";
    out.close();*/


    glTranslated(pTemp.x(),pTemp.y(), 0);//?? ACE ??
    ...
    case 0:
        drawAction_drawVehicleAsQuadPlus(*this, upscale);
        break;
    case 1:
   ...
   /* //?? ACE ?? get dump ...!?!?!?
    OutputDevice_File gui_dump = new OutputDevice_File("gui_dump.xml");
    gui_dump.openTag("timestep") << " time=\"" << time2string(timestep) << "\">\n";
    gui_dump.openTag("edge") << " id=\"" << edge.getID() << "\">\n";*/
}
...
//?? ACE ??
size_t
GUIVehicle::getMainStripNumericalID() const throw(){
	myLock.lock();
	size_t ret = MSVehicle::getMainStripNumericalID();
	myLock.unlock();
	return ret;
}
...
//?? ACE ??
bool
GUIVehicle::isMainStrip(const MSStrip &strip) const {
	myLock.lock();
	bool ret = MSVehicle::isMainStrip(strip);
	myLock.unlock();
	return ret;
}

SUMOReal
GUIVehicle::getRealWidth() const throw(){
	myLock.lock();
	SUMOReal ret = MSVehicle::getRealWidth();
	myLock.unlock();
	return ret;
}//?? ACE ??
