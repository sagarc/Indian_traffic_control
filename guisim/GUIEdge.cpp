...
/// @date    Sept 2002
/// @version $Id: GUIEdge.cpp 8725 2010-05-04 20:27:48Z behrisch $
///
...
GUIEdge::drawGL(const GUIVisualizationSettings &s) const throw() {
    ...
    // draw strips ?? ACE ??
        if (s.scale>1.&&myFunction!=MSEdge::EDGEFUNCTION_INTERNAL) {
            // draw white boundings
            glTranslated(0, 0, .004);
            //glColor3d( 1, 1, 1);
            for (LaneWrapperVector::const_iterator i=myLaneGeoms.begin(); i!=myLaneGeoms.end(); ++i) {
                Position2DVector temp = (*i)->getShape();
                for(int j = 0;j < ((*i)->getLane().getWidth());j++){
                	if (j == 0){
                		temp.move2side((SUMOReal)+1.6 - (SUMO_const_laneWidth/(*i)->getLane().getWidth()));
                	}
                	else{
                		temp.move2side((SUMOReal) - (SUMO_const_laneWidth/(*i)->getLane().getWidth()));
                	}
                    glColor3d(0.2,0.2,0.2);
                    GLHelper::drawLine(temp);
                }
            }
            glTranslated(0, 0, -.004);
        }
