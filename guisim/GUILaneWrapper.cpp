...
/// @version $Id: GUILaneWrapper.cpp 8725 2010-05-04 20:27:48Z behrisch $
///
...
GUILaneWrapper::drawGL(const GUIVisualizationSettings &s) const throw() {
    ...
    if (s.scale>s.minVehicleSize) {
        // retrieve vehicles from lane...//strip//...; disallow simulation //?? ACE ??
    	int i = 0;//FIXME:temporary fix ....actually should use it->getNumericalID() instead of i//?? ACE ??
    	for (MSLane::StripContConstIter it = myLane.myStrips.begin(); it != myLane.myStrips.end(); ++it,++i) {//?? ACE ??
    		const MSLane::VehCont &vehicles = myLane.getVehiclesSecure(i);//?? ACE ??

    		for (MSLane::VehCont::const_iterator v=vehicles.begin(); v!=vehicles.end(); ++v) {
    		        	//std::string vehID = static_cast<const GUIVehicle*const>(*v)->getID();//?? ACE ??
    		        	//std::cout << "\n guilanewrapper:vehid:" << vehID ; //?? ACE ??
    			if((*v)->isMainStrip(**it))
