...
/// @date    Thu, 04.09.2003
/// @version $Id: GUIInternalLane.cpp 8617 2010-04-21 15:06:43Z behrisch $
///
...
GUIInternalLane::GUIInternalLane(const std::string &id,
                                 SUMOReal maxSpeed, SUMOReal length, size_t stripWidth,
                                 MSEdge * const edge, unsigned int numericalID,
                                 const Position2DVector &shape,
                                 const std::vector<SUMOVehicleClass> &allowed,
                                 const std::vector<SUMOVehicleClass> &disallowed) throw()
        : MSInternalLane(id, maxSpeed, length, stripWidth, edge, numericalID, shape, allowed, disallowed) {}
...
GUIInternalLane::isEmissionSuccess(MSVehicle* aVehicle, SUMOReal speed, SUMOReal pos,
    ...
    myLock.lock();
    bool ret = MSInternalLane::isEmissionSuccess(aVehicle, speed, pos, recheckNextLanes, startStripId);
    myLock.unlock();
    return ret;
}
...
GUIInternalLane::getVehiclesSecure() const throw() {//?? ACE ?? commented
    myLock.lock();
    return myVehicles;
}

const MSLane::VehCont &
GUIInternalLane::getVehiclesSecure(int i) const throw() {
    myLock.lock();
    /*std::cout<<"GuiInL...vehsecure :"<<i<<"\n";*/
...
bool
GUIInternalLane::push(MSVehicle* veh, const std::vector<int> &stripID, bool hasMainStrip) {
    // Insert vehicle only if it's destination isn't reached.
    //  and it does not collide with previous
    // check whether the vehicle has ended his route
    myLock.lock();
    try {
        MSLane::push(veh, stripID, hasMainStrip);
        myLock.unlock();
        return false;
    } catch (ProcessError &) {
        myLock.unlock();
