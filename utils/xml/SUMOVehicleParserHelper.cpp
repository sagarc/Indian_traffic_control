...
/// @date    Mon, 07.04.2008
/// @version $Id: SUMOVehicleParserHelper.cpp 8732 2010-05-05 13:17:09Z behrisch $
///
...
SUMOVehicleParserHelper::beginVTypeParsing(const SUMOSAXAttributes &attrs) throw(ProcessError) {
    ...
    if (attrs.hasAttribute(SUMO_ATTR_VEHSTRIPWIDTH)) {
        vtype->stripWidth = (size_t) attrs.getIntReporting(SUMO_ATTR_VEHSTRIPWIDTH, "vtype", vtype->id.c_str(), ok);
        vtype->setParameter |= VTYPEPARS_STRIPWIDTH_SET;
    } else {
        vtype->stripWidth = DEFAULT_VEH_STRIPWIDTH;
    }
...
SUMOVehicleParserHelper::parseVTypeEmbedded(SUMOVTypeParameter &into,
	...
	/*std::cout << "parsing Krauss" << std::endl;*/
	...
	//std::cout << "Parsing IITBNN params" << std::endl;
        parseVTypeEmbedded_IITBNN(into, attrs);
        break;
    }
    if (!fromVType) {
...
void
SUMOVehicleParserHelper::parseVTypeEmbedded_IITBNN(SUMOVTypeParameter &into,
        const SUMOSAXAttributes &attrs,
        bool fromVType) throw(ProcessError) {
    bool ok = true;
    if (attrs.hasAttribute(SUMO_ATTR_ACCEL)) {
        into.cfParameter["accel"] = attrs.getSUMORealReporting(SUMO_ATTR_ACCEL, "IITBNN", into.id.c_str(), ok);
    }
    if (attrs.hasAttribute(SUMO_ATTR_DECEL)) {
        into.cfParameter["decel"] = attrs.getSUMORealReporting(SUMO_ATTR_DECEL, "IITBNN", into.id.c_str(), ok);
    }
    if (attrs.hasAttribute(SUMO_ATTR_SIGMA)) {
        into.cfParameter["sigma"] = attrs.getSUMORealReporting(SUMO_ATTR_SIGMA, "IITBNN", into.id.c_str(), ok);
    }
    if (attrs.hasAttribute(SUMO_ATTR_TAU)) {
        into.cfParameter["tau"] = attrs.getSUMORealReporting(SUMO_ATTR_TAU, "IITBNN", into.id.c_str(), ok);
    }
    if (!ok) {
        throw ProcessError();
    }
